...
class JingxiDeliveryTime extends Component {
  ...
  dayRange (type) {
    ...
    const date4 = new Date(date0.setDate(date0.getDate() + 1))
    const date5 = new Date(date0.setDate(date0.getDate() + 1))
    return [
      {enabled:false,value:date1,selected:false},
      {enabled:type==='take',value:date2,selected:type==='take'},
      {enabled:false,value:date3,selected:false},
      {enabled:false,value:date4,selected:false},
      {enabled:type==='delivery',value:date5,selected:type==='delivery'},
    ]
  }
  ...
  okDeliveryTimePicker(obj){
    const value = `afterAfterAfterTomorrow|${obj.time}`
...
export default JingxiDeliveryTime
